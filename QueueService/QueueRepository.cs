    ...
    public class QueueRepository
    {
        ...
        public static Tuple<bool, int> InsertUpdateTable(Dictionary<string, string> payload)
        {
            try
            {
                ...
                else if (tableName == "sms_config")
                {
                    success = UpdateSmsConfig(paramDict, out affected);
                    return Tuple.Create(success, affected);
                }
                else
        ...
        public static bool UpdateSmsConfig(Dictionary<string, string> parameter, out int affected)
        {
            try
            {
                ...
                if (Database.Me.Connected)
                {
                    string command = parameter["command"];
                    string jsonData = parameter["data"];
                    Dto.SmsConfig sms = (Dto.SmsConfig)JsonConvert.DeserializeObject(jsonData, (typeof(Dto.SmsConfig)));
                    string sql = "UPDATE sms_config SET endpoints = '" + sms.endpoints + "', username = '" + sms.username + "', password = '" + sms.password + "' WHERE id = 1";
        ...
        public static bool UpdateTokenActive(string username, string token , string post)
        {
            try
            {
                ...
                if (Database.Me.Connected)
                {
                    ...
                    string sql = "UPDATE logins SET last_login = '"+ strDate + "', firebase_token = '"+token+"', post_code = '"+post+"' WHERE username = '"+username+"'";
        ...
        public static string GetTotalWaitingJobs(string post)
        {
            try
            {
                if (Database.Me.Connected)
                {
                    ...
                    string query = string.Format(@"SELECT id, number, status, post, source, date, count(*) as jumlah FROM jobs
                                      WHERE status = 'WAITING' AND date = date('now','localtime') AND post = '{0}' ", post);

                    Database.Me.OpenConnection();

                    DataTable dataTable = Database.Me.CreateDataTable(query);
                    string jsonDataTable = JsonConvert.SerializeObject(dataTable, Formatting.None);
                    if (jsonDataTable == "[]")
                        jsonDataTable = "";
        ...
        public static DataTable GetLoginsPost(string post)
        {
            try
            {
                if (Database.Me.Connected)
                {
                    ...
                    string query = string.Format(@"SELECT id, username, last_login, post_code FROM logins
                                      WHERE post_code = '{0}'", post);
        ...
        public static string GetJob(Dictionary<string, string> parameter)
        {
            try
            {
                ...
                if (status == "PROCESS" || status == "FINISHED" || status == "ALL")
        ...
        public static string GetJob(string post, string status, int offset = 0, int limit = 0)
        {
            if (Database.Me.Connected)
            {
                try
                {
                    ...
                    {
                        if(status == "ALL") {
                            sql = @"SELECT id, number, status, station, post, source, date, starttime, calltime, call2time, endtime FROM jobs
                                      WHERE date = date('now','localtime')
                                      AND post = ? ORDER BY starttime DESC";
                        } else {
                            sql = string.Format(@"SELECT id, number, status, station, post, source, date, starttime, calltime, call2time, endtime FROM jobs
                                      ...
                                      AND post = ? ORDER BY starttime DESC", status);
                        }
        ...
        */
        ...
        #region Huawei HiLink DB Connection
            public static DataTable getConfigModem()
            {
                try
                {
                    if (Database.Me.Connected)
                    {
                        ...
                        string query = @"SELECT * FROM sms_config";
            ...
            public static string getConfigModemStr()
            {
                try
                {
                    if (Database.Me.Connected)
        ...
        public static Dictionary<string, string> getConfigModemString(Dictionary<string, string> parameter)
            {
                try
                {
                    if (Database.Me.Connected)
