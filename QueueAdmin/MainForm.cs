    ...
    public partial class MainForm : Form
    {
        ...
        private void NetSessionDataReceived(DataReceivedEventArgs arg)
        {
            ...
            else
            {
                ...
                {
                    //MessageBox.Show(arg.DataString);
                    HandleMessage(arg);
                }
                else
                {
        ...
        private void HandleMessage(DataReceivedEventArgs arg)
        {
            try
            {
                ...
                else if (qmessage.MessageType == Msg.SysGetTable && qmessage.Direction == MessageDirection.RESPONSE)
                {
                    ...
                    else if (tableName == Tbl.sms)
                    {
                        _tableProps[tableName].TotalRows = totalRowInt;
                        tbReordStatus.Text = _tableProps[tableName].NavigationStatus;
                        btnDeleteData.Enabled = totalRowInt > 0;
                        DataTable table = JsonConvert.DeserializeObject<DataTable>(jsonDataTable);
                        foreach (DataRow row in table.Rows)
                        {
                            textBox3.Text = row["endpoints"].ToString();
                            textBox6.Text = row["username"].ToString();
        ...
        private void getModemConfig()
        {
            if (_client != null && _client.Connected)
            {
                //SYS | GET_TABLE | REQ | Identifier |[Table!Param0!Param1]
                string message = Msg.SysGetTable.Text +
                                 Msg.Separator + "REQ" +
                                 Msg.Separator + "Identifier" +
                                 Msg.Separator + "sms_config" +
                                 Msg.CompDelimiter + "0" +
                                 Msg.CompDelimiter + "1";
        ...
        private void testConnectionModem()
        {
        ...
        {
            disableButton();
            sendSMS(textBox8.Text,textBox9.Text);
            enableButton();
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            Dto.SmsConfig sms = new Dto.SmsConfig()
            {
                endpoints = ip,
                username = username,
                password = password,
            ...
            string jsonSms= JsonConvert.SerializeObject(sms, Formatting.None);
            ...
            string message = Msg.SysUpdTable.Text +
                             Msg.Separator + "REQ" +
                             Msg.Separator + "Identifier" +
                             Msg.Separator + Tbl.sms +        // tablename
